...
local function SafeGet(key: string, error_msg: string, show_error: bool, player: Object, kickOnError: bool, timeout: number)
...
local function SafeSet(key: String, data: any, error_msg: string, show_error: string, player: Object, kickOnError: bool, timeout: number)
...
local function OnPlayerAdded(player)
	...
	local data = SafeGet(player.UserId.."-"..Key,"Fastr: error loading moderation data, please rejoin",player,true)
	...
	if data then
		...
		if data.Bans[1].ban_time[1] > tick() and data.Bans[1].ban_repealed == false then
			print("KICK")
			player:Kick("you have been banned for "..data.Bans[1].ban_time[2].." days by "..data.Bans[1].moderator..". \n Moderator note: "..data.Bans[1].mod_note)
		else
			print("no kick")
		end
	end
...
DSLib.Ban = function(player,days,user,note)
	...
	local data = SafeGet(player.UserId.."-"..Key,"Fastr: error getting player moderation data for ban. Moderation data may be wiped.",false,user,false,250)
	...
	if data then
		...
		if player.UserId ~= game.CreatorId then
			...
			SafeSet(player.UserId.."-"..Key,data,"Error occured while banning player. Please try again.",true,user,false,250)

			player:Kick("you have been banned for "..days.." days by "..data.moderator..".\n Moderator note: "..data.mod_note)
		...
		if player.UserId ~= game.CreatorId then
			...
			SafeSet(player.UserId.."-"..Key,DataTable,"Error occured while banning player. Please try again.",true,user,false,250)

			player:Kick("you have been banned for "..days.." days by "..DataTable.Bans[1].moderator..".\n Moderator note: "..DataTable.Bans[1].mod_note)
